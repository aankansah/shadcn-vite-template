...
export default function BottomNavigation() {
  ...
  return (
    <footer className='relative h-(--footer-height) mx-auto z-20'>
      ...
      <div className='w-3/5 mx-auto h-full absolute z-20 inset-0 flex items-center justify-between gap-12'>
        {items.map((item) => {
          const active = location.pathname === item.path
          return (
            <Link key={item.path} to={item.path} className='w-28 relative flex flex-col items-center justify-center gap-2 h-full'>
              <img src={active ? item.icon.active: item.icon.inactive} alt={item.label} className={cn('relative z-20 size-8', {
                'size-8' : active,
                'text-white': !active
              })} />
              <span className={cn('relative z-20 text-xs', {
